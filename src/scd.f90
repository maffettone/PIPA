  ! LAST EDIT: Phil Maffettone 2017-04-28
module scd
  ...
  character(len=100),parameter :: geom_lib= '/Users/alggroup/Documents/Development/PIPA/lib/protein.amber94.edit.pdb'                            !Using AMBER parm94 for geometry from pdb file
  character(len=100),parameter :: chi_lib='/Users/alggroup/Documents/Development/PIPA/lib/sc_dih.lib'                             !Using table from ccp14
  character(len=100),parameter :: vdw_lib='/Users/alggroup/Documents/Development/PIPA/lib/vdwr.lib'                             !van der Waals radii library
...
contains
  ...
  subroutine decorate(prot)
    ...
    call relax_sc(prot%pep,adj,chi_rels)
  end subroutine decorate
  ! -----------------------------------------------------------------------------

  subroutine relax_sc(pep,adj,chi_rels)
    ...
    integer :: maxmoves                                     !Number of MC moves
    ...
    !------------------------------------ PARAMETER SET----------------------------------------
    ! Pulling in van der Waals radii
    info=0
  ...
  subroutine add_sc(pep)
    ...
    do i=1,size(pep)
       ...
       do j=1,size(fragments)
          ...
          else if(trim(fragments(j)%res_label) == 'HID' .and. trim(pep(i)%label)=='HIS') then
             r1 = coords - fragments(j)%CA%coords
             if(allocated(pep(i)%sidechain)) deallocate(pep(i)%sidechain)
             allocate(pep(i)%sidechain(size(fragments(j)%atoms)))
             do k=1,size(fragments(j)%atoms)
